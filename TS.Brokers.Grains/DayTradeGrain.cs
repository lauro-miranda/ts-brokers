...
{
    [StorageProvider(ProviderName = "dayTradeStore")]
    public class DayTradeGrain : Grain<DayTradeState>, IDayTradeGrain
    {
        IClusterClient ClusterClient { get; }

        public DayTradeGrain(IClusterClient clusterClient)
        {
            ClusterClient = clusterClient;
        }

        public async Task<Response> Purchase(DayTradeRequestMessage message)
        {
            ...
            var asset = message.CustomerToAssetState(price);
            ...
            {
                State = new DayTradeState { Identification = message.Identification };
            }
        ...
        public async Task<Response> Sell(DayTradeSaleRequestMessage message)
        {
            var response = Response.Create();

            State.Assets[message.Symbol].RemoveRange(0, message.Quantity);

            var assetsQuantity = State.Assets[message.Symbol].Count;
        ...
        public Task<DayTradeState> Get() => Task.FromResult(State);
